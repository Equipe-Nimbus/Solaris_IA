...
def evaluate_and_compare(image_paths, mask_paths, predict_paths, device, image_size=(747, 768)):
    ...
    for img_path, mask_path, predict_path in tqdm(zip(image_paths[:130], mask_paths[:130], predict_paths[:130]), total=130, desc="Avaliando"):
...
def main(model_path, image_dir, mask_dir):
    ...
    there_is_not_pred = len([os.path.join(image_dir, f) for f in os.listdir(predict_dir) if f.endswith('.png')][:130]) == 0
    # Listar as imagens e as máscaras (carregar até 130)
    if(there_is_not_pred):
        run_predict("Modelo/checkpoints/checkpoint_epoch40.pth", "avaliacao/imgs", "avaliacao/previsao", False, 0.5, (256, 256), False, 2, True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    predict_paths = [os.path.join(predict_dir, f) for f in os.listdir(predict_dir) if f.endswith('.png')][:130]
    image_paths = [os.path.join(image_dir, f) for f in os.listdir(image_dir) if f.endswith('.png')][:130]
    mask_paths = [os.path.join(mask_dir, f) for f in os.listdir(mask_dir) if f.endswith('.png')][:130]
...
if __name__ == '__main__':
    model_path = './Modelo/checkpoints/checkpoint_epoch40.pth'  # Substitua pelo caminho do seu modelo
